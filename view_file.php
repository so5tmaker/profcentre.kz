...
$result = mysql_query("SELECT * FROM files WHERE id='$id_score'",$db);
...
if (mysql_num_rows($result) > 0)
{
    ...
    $update = mysql_query ("UPDATE files SET view='$new_view' WHERE id='$id_score'",$db);
}
else
{
      ...
      <tr>
       <? include ("blocks/lefttd.php"); ?>
        <td valign='top'>
		 ...
		 <?
		 printf ("<p class='post_title2'>%s</p><p class='post_add'>Автор: %s</p><p class='post_add'>Дата: %s</p>%s",$myrow["title"],$myrow["author"],$myrow["date"],$myrow["text"]);
         printf ("<br><p class='post_add'><img class='down' align='left' src='files/download.gif'></p><p><a target='_blank' href='%s'>Скачать \"%s\"</a></p><p class='post_view'>Просмотров: %s</p>",$myrow["file"],$myrow["title"],$myrow["view"]);

         ?>

<form action="vote_res.php" method="post" name="vv">
<p class="pvote">Оцените закачку: 1 <input name="score" type="radio" value="1"> 2 <input name="score" type="radio" value="2"> 3 <input name="score" type="radio" value="3"> 4 <input name="score" type="radio" value="4"> 5 <input name="score" type="radio" value="5" checked>
<input class="sub_vote" name="submit" type="submit" value="Оценить">
<input name="id" type="hidden" value="<?php echo "$id_score";?>">
<input name="tbl_name" type="hidden" value="<?php echo "files";?>">
...
<form action="comment.php" method="post" name="form_com">
    ...
    <img style='margin-top:5px; align:left' src="<? echo $myrow4["img"]; ?>"><br>
      ...
      <input name="it_is_file" type="hidden" value=1>
