...
{
   $slash_txt = addslashes($text);
/* Здесь пишем что можно заносить информацию в базу */
$result = mysql_query ("UPDATE ".$tbl_dt." SET title='$title', meta_d='$meta_d', meta_k='$meta_k', date='$date', description='$description', text='$slash_txt', author='$author', cat='$cat', mini_img='$img', secret='$secret' WHERE id='$id'");
if ($result == 'true') {echo "<p>Обновление ".$name_dt." прошло успешно!</p>";}
else {echo "<p>Обновление ".$name_dt." не прошло!".mysql_error()."ррпрпррп</p>";}
}		 
else 
{
